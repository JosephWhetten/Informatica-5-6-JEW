def main():
    ...
    if choice == 2:
        ...
        find_largest(read_numbers(filename))
...
def find_largest(num: list):
    new_num = []
    for i in num:                            # Converts the numbers as strings into integers
        new_num.append(int(i))
    print(f"{max(new_num)} is the max")

def read_numbers(file_name):
    with open(file_name, "r") as file:
        lines = file.readlines()                        # Gets every line in the file
    numbers = []
    for line in lines:
